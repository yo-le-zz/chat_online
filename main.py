...
if missing:
    ...
    print(colorama.Fore.GREEN + "[OK] Toutes les variables d'environnement sont présentes.")

# ------------------- CRYPTO / HASH -------------------
fernet_key = os.getenv("FERNET_KEY")
if not fernet_key:
    print("FERNET_KEY manquante ou vide.")
    sys.exit(1)
...
def open_ssh_tunnel():
    ...
    try:
        tunnel = SSHTunnelForwarder(
            (ssh_host, 22),  # <-- ici ssh_host = os.getenv("IP_SERVEUR")
            ssh_username=ssh_user,  # <-- ssh_user = os.getenv("SSH_USER")
            ssh_password=ssh_password,  # <-- ssh_password = os.getenv("SSH_PASSWORD")
            remote_bind_address=('127.0.0.1', 5432),
            local_bind_address=('127.0.0.1', 6543)
        )
        tunnel.start()
        print(colorama.Fore.YELLOW + f"✅ Tunnel SSH ouvert sur le port local {tunnel.local_bind_port}")

        connection = connect_to_db(
            host='127.0.0.1',
            database=db_name,       # <-- db_name = os.getenv("DATABASE")
            user=db_user,           # <-- db_user = os.getenv("DB_USER")
            password=db_password,   # <-- db_password = os.getenv("DB_PASSWORD")
            port=tunnel.local_bind_port
        )
        if connection is None:
            print(colorama.Fore.RED + "❌ Aucune connexion active à la base. Vérifie le tunnel SSH et les identifiants.")
            exit(1)

        if connection:
            print(colorama.Fore.GREEN + "✅ Connexion PostgreSQL établie via SSH.")
        else:
            print(colorama.Fore.RED + "❌ Échec de la connexion à PostgreSQL via SSH.")

        return tunnel, connection
    except Exception as e:
        print(colorama.Fore.RED + f"Erreur lors de l'ouverture du tunnel SSH: {e}")
        return None, None
...
if __name__ == "__main__":
    ...
    if not connection:
        print(colorama.Fore.RED + "❌ Impossible d'établir la connexion SSH/DB. Fermeture du programme.")
        exit(1)
